...
uint8_t valid_count = 0;                     // 光电管亮起数量
uint8_t *valid_count_address = &valid_count; // 光电管亮起数量地址
...
{ // 光电管误差计算函数
  ...
  weighted_sum_record = weighted_sum; // 记录光电管误差并对其进行滤波处理,除以亮起数量取平均，是为了防止亮起数量变化导致误差突变
